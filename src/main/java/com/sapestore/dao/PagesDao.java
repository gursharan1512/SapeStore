...
public class PagesDao {
  ...
  private  static final SapeStoreLogger LOGGER = SapeStoreLogger
      .getLogger(PagesDao.class.getName());
  ...
  public String getContactUs() throws SapeStoreException {
    ...
    try {
      miscelleneousList = (List<Miscelleneous>) hibernateTemplate
          .findByNamedQuery("Miscelleneous.findAll");
      if (!miscelleneousList.isEmpty()) {
        contactUsText = miscelleneousList.get(0).getContactUs();
      }
    } catch (SapeStoreSystemException dbe) {
      LOGGER.fatal(
          "A DB exception occured while getting the contact us details", dbe);
      throw dbe;
    }
  ...
  public String getPolicy() throws SapeStoreException {
    ...
    try {
      ...
      LOGGER.fatal("A DB exception occured while getting the policy details",
          dbe);
      throw dbe;
    }
  ...
  public void setContactUs(String text) throws SapeStoreException {
    ...
    try {
      ...
      LOGGER.fatal("A DB exception occured while saving the contact us details",
          dbe);
      throw dbe;
    }
