...
function changeVal()
{
			...
			<section>
				<div class="leftCol">
         		...
         		class="highlighted"
								...
								<a style="font-weight: bold;" href="editInfo"
								title="personalinformation">Personal Information </a>
							</li>
							<li
	...
	function checkphone() {
	    ...
	    if(x.length<10){
	    	document.getElementById('phonecheck').innerHTML="Enter 10 digit phone number";
	    }else{
	    	re = /[0-9]/;
	        if(!(re.test(registerForm.mobileNumber.value))) {
	        	document.getElementById('phonecheck').innerHTML="Invalid Input";
	           }
	...
	function populateCity(stateId){
	    ...
	    function checkmobile() {
	        ...
	        if(x.length<10){
	        	document.getElementById('mobcheck').innerHTML="Enter 10 digit mobile number";
	        }else{
	        	re = /[0-9]/;
	            if(!(re.test(registerForm.mobileNumber.value))) {
	            	document.getElementById('mobcheck').innerHTML="Invalid Input";
	    ...
	    function count() {
	        ...
	        if(!(re.test(Myform.postalCode.value))) {
	        	document.getElementById('zipcheck').innerHTML="Zip code contain only numbers";
	        	}
	    }
		...
		<div id="mainContent">
			...
			<%-- <form:form action="/SapeStore/updateProfile" id="Myform"
				 method="POST" commandName="profilevo" name="Myform">
				<table id="tablepaging" class="yui"
					style="width: 650px; height: 91px; border-bottom-width: 0px">
					<tr>
						<td><label> <strong>fullname</label><font color="red">*</font>
						</td>
						<td><input id="fnm" type="text" name="name" value="${profilevo.name}"  /><form:errors path="name"
											cssClass="error" /></td>
					</tr>
					<tr>
						<td><label> <strong>loginname</strong></label> <font color="red">*</font>
						</td>
						
						<td><input id="tx" type="text" name="userId" readonly="readonly"  value="${profilevo.userId}"/><form:errors path="userId"
											cssClass="error" /></td>
					</tr>
					<tr>
						<td><label> <strong>loginpassword</label><font color="red">*</font>
						</td>
					...
					<tr>
						<td><label> <strong>retypepassword</strong></label> <font color="red">*</font>
						</td>
						<td><input id="tx" type="password" required name="cfmpassword" placeholder="Retype Password" onblur="checkpassword()"/><div id="repass" style="color: red; font-size: 2"></div></td>
					</tr>
					<tr>
						<td><label> <strong>email</strong></label> <font color="red">*</font>
						</td>
						<td><input id="tx" type="email" name="emailAddress" value="${profilevo.emailAddress}" readonly="readonly"/></td>
					</tr>
					<tr>
				
					
						<td><label> <strong>addressline1</strong></label><font color="red">*</font>
						</td>
						<td><input id="tx" type="text" name="addressLine1" value="${profilevo.addressLine1}"><form:errors path="addressLine1"
											cssClass="error" /></td>
					</tr>
					<tr>
						<td><label> <strong>addressline2</strong></label></td>
						<td><input id="tx" type="text" name="addressLine2" value="${profilevo.addressLine2}"/></td>
					</tr>
					<tr>
						<td><label> <strong>state</strong></label><font color="red">*</font>
						</td>
						<td><select name="stateId" id="list" onchange="populateCity(this.value)">
                     ...
                     </select><form:errors path="stateId"
											cssClass="error" /></td>
					</tr>
					<tr>
						<td><label> <strong>city></strong></label><font color="red">*</font>
						</td>
						<td><div id="populateCity"><select name="cityId" id="list" >
                     ...
                     </select></div><form:errors path="cityId"
											cssClass="error" /></td>
					</tr>
					
					<tr>
						<td style="font-size: 13px;"><label> <strong>zipcode</strong></label><font color="red">*</font>
						</td>
						<td><input id="zp" type="text" name="postalCode" value="${profilevo.postalCode}" maxlength="6" onblur="count()"/><form:errors path="postalCode"
											cssClass="error" /></td>
					</tr>
					<tr>
						<td><label> <strong>phonenumber</strong></label></td>
						<td><input id="tx" type="text" name="phone" value="${profilevo.phone}" maxlength="10" onblur="checkphone()"/><form:errors path="phone"
											cssClass="error" /></td>
					</tr>
					<tr>
						<td> <strong>mobilenumber</strong></label></td>
						<td><input id="tx" type="text" name="mobileNumber" value="${profilevo.mobileNumber}" maxlength="10" onblur="checkmobile()"/><form:errors path="mobileNumber"
											cssClass="error" /></td>
					</tr>
				</table>
				<br>
				<div style="width: 200px; margin-left: auto; margin-right: auto;">
				<spring:message code="label.updateprofile" var="labelupdateprofile"></spring:message>
					<input id="but" name="but" type="submit" value="update profile" >
				</div>
			</form:form> --%>
			<form:form action="/SapeStore/updateProfileform" id="Myform"
				method="POST" commandName="profilevo" name="Myform">
				<fieldset></fieldset>
				<table class="wwFormTable" cellpadding="10">
					<tr>
						<td><label id="name">Full Name</label> <output
								style="color: red; vertical-align: super; font-size: small;"></output></td>
						<td>${profilevo.name}</td>
					</tr>
					<tr>
						<td><label> Login Name</label> <font color="red"></font></td>

						<td>${profilevo.userId}</td>
					</tr>
					<tr>
						<td><label id="password">Password</label> <output
								style="color: red; vertical-align: super; font-size: small;"></output></td>
						<td>******</td>
					</tr>
					...
					<tr>
						<td><label id="address1">Address Line 1</label> <output
								style="color: red; vertical-align: super; font-size: small;"></output></td>
						<td>${profilevo.addressLine1}</td>
					</tr>
					<tr>
						<td><label id="address2">Address Line 2</label></td>
						<td>${profilevo.addressLine2}</td>
					</tr>
					<tr>
						<td><label> State</label><font color="red"></font></td>
						<c:if test="${not empty cityName }">
							<td>${stateName }</td>
						</c:if>
					</tr>
					<tr>
						<td><label> City</label><font color="red"></font></td>
						<c:if test="${not empty cityName }">
							<td>${cityName }</td>
						</c:if>
					</tr>
					...
					<tr>
						<td><label id="zip">Zip Code</label> <output
								style="color: red; vertical-align: super; font-size: small;"></output></td>
						<td>${profilevo.postalCode}</td>
					</tr>
					<tr>
						<td><label id="phNo">Phone Number</label></td>
						<td>${profilevo.phone}</td>
					</tr>
					<tr>
						<td><label id="mobNo">Mobile Number</label></td>
						<td>${profilevo.mobileNumber}</td>
					</tr>
					<tr>
						<td></td>
						<td style="text-align: left;"><div>

								<input type="submit" value="Edit Profile" class="lightButton"
