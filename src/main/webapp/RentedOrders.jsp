...
<script type="text/javascript">
		var pager = new Pager('tablepaging1', 10);
		pager.init();
...
<script>
	...
	function sendOrderList(){
		...
		document.updateDispatch1.submit();
...
<div id="replacediv">
	<form:form method="GET" action="/SapeStore/updateRentDispatch"
		...
		<h3 class="sapfont">Here is the list of all the Rented Orders showing their dispatched status and return status.</h3>
		<table id="tablepaging1" class="yui"
			...
			<c:forEach items="${rentedOrdersList}" var="current" varStatus="loop">
				<tbody>
					<tr id="index${current.orderId}">
						...
						<c:set var="returnReceivedVar" value="${current.returnStatus}" />
						<%
									Boolean returnReceivedVar = (Boolean) request
													.getAttribute("returnReceivedVar");
								%>
						<td><c:choose>
								<c:when test="${current.returnStatus=='Returned'}">
									<input type="checkbox" id="returnCheckIndex${current.orderId}" 
										checked disabled onclick="returnClick(this)" />
								</c:when>
								<c:otherwise>
									<input type="checkbox" id="returnCheckIndex${current.orderId}" disabled
										onclick="returnClick(this)" />
								</c:otherwise>
							</c:choose></td>

						<td id="dispatchTextIndex${current.orderId}"><input
							type="text" style="display: none;" value="${current.orderStatus}"
							name="rentedUpdateList[${loop.index}].dispatchStatus"></td>

						<td id="returnTextIndex${current.orderId}"><input type="text"
							style="display: none;" value="${current.returnStatus}"
							name="rentedUpdateList[${loop.index}].returnStatus"></td>
					</tr>
				</tbody>
			</c:forEach>
		</table>
 ...
 -->
		<div
			...
			<input type="button" onclick="sendOrderList()" name="dispatch" class="btn"
		...
		<div id="replace"
			...
			<input type="button" name="return" style="font-family: Georgia;" class="btn"
				id="return" value="RETURN RECEIVED"
				onclick="location.href ='/SapeStore/trackReturn'" />
		</div>
	...
	<form:form name="updateDispatch1" action="/SapeStore/updateRentDispatch" method="GET" >
					<input type="hidden" name="orderList" id="orderList"/>
					<input type="button" id="braavos1" hidden="hidden" >
				</form:form> 
