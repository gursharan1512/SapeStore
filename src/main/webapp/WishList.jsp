...
function changeVal()
{
	...
	<div id="wrapper">
					...
					<c:choose>
						...
						<c:otherwise>
							<select id="account" name="account" style="font-size: 12px;"
								...
								<option value="-1" style="font-size: 10px;">Welcome
									${username}</option>
								<option value="5" style="font-size: 10px;">Edit Profile</option>
								<option value="5" style="font-size: 10px;">Order Status
									Tracking</option>
		...
		<div id="mainContent">
			...
			<c:choose>
				<c:when test="${empty wishListArr}">
					<td><h3>Nothing to show in your wishlist</h3></td>
				</c:when>
				<c:otherwise>
					<form:form name="form" action="addToCartFromWishList"
						modelAttribute="wishArr" commandName="wishArr">
						...
						<table id="tablepaging" class="yui">
							...
							<c:forEach items="${wishListArr}" var="wishArr" varStatus="loop">


								<tr>
									<td style="width: 120px;"><img
										src="${wishArr.bookThumbImage}" height="40" width="30"></td>
									<td id="product_name"
										style="width: 120px; word-break: break-word">${wishArr.bookTitle}
									</td>
									<td style="width: 110px; word-break: break-word">${wishArr.isActive}</td>
									<td style="width: 110px; word-break: break-word">${wishArr.bookPrice}</td>

									<td><input type="button" id="submit-${loop.index}"
										class="button"
										onclick="registerEdit('${loop.index}','${wishArr.wishId}');"></input>
									<td><div id="divinline-${loop.index}"
											style="width: 80px; height: 80px; display: none; color: green;">
											<strong><h3>ADDED TO CART</h3></strong>
										</div> <%-- <input type="hidden" name="wishArrISBN" value="${wishArr.isbn}" id="wishArrISBN-${loop.index}"> --%>

										<%-- <a href="/addToCartFromWishList" id="${counter}" name="${counter}"><img src="img/add_cart.jpg"></a> --%>
									</td>
			...
			<script type="text/javascript">
					function registerEdit(index,wishId) {
						//get chequeStatus value like this
						var url = "/SapeStore/addToCartFromWishList?wishId="
								+ wishId;
						...
						$("#divinline-"+index).fadeIn();

						/* window.location.replace("WishList.jsp");  */
						}
